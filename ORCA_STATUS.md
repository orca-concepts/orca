...
**Last Updated:** April 30, 2026 (Phase 55d ORCID OAuth callback back-button trap COMPLETE — added `exchangedRef` guard to prevent double code exchange when `refreshUser()` changes the `user` effect dependency; both `navigate` calls already had `replace: true`. Prior: Phase 55b seed `question` attribute in migrate.js COMPLETE — fixes missing [question] filter on fresh production deploys; migrate.js seed audit found no other drift. Phase 55a DATABASE_URL connection string for managed Postgres providers COMPLETE — `database.js` uses `DATABASE_URL` with SSL when set, falls back to individual `DB_*` env vars for local dev. Prior: Phase 54 production deployment readiness COMPLETE — 54a skipped (no file storage to migrate — documents store extracted text in `documents.body`, not files on disk; multer already uses `memoryStorage()`), 54b backend serves frontend static files in production via `express.static` + SPA fallback (`/^(?!\/api).*/` regex), new root `package.json` with `build` and `start` scripts (migrations run automatically on startup via `start`), 54c license consistency `AGPL-3.0-only` in all three `package.json` files + production config audit confirmed (localhost refs are all dev fallbacks behind env vars, no cookies/sessions used, trust proxy correctly set to 2). Phase 53 also COMPLETE — all sub-phases shipped.)

**Prior Last Updated:** April 28, 2026 (Phase 53 DMCA & illegal-content removal infrastructure COMPLETE. Prior: April 25, 2026 (Phase 52 data portability/correction COMPLETE — 52a self-service `GET /api/users/me/export` endpoint returning JSON of account + contributions + votes/subscriptions, with `data_export_requests` audit table and 2-per-12-months rate limit; 52b+c `PATCH /api/users/me` email correction endpoint + Privacy & Data section on profile page with download button, exports-used counter, inline email edit, and privacy contact mailto; follow-up commit softened user-facing strings to drop the explicit "Colorado Privacy Act" reference in favor of plain language. 52d data subject request form intentionally deferred. Phase 51 click-wrap consent also COMPLETE — schema + backend verifyRegister consent fields + standalone backfill script + static `/terms` + `/privacy` placeholder pages + LoginModal Sign Up click-wrap.)
...
**Note on placeholder content:** The `/terms` and `/privacy` pages currently display placeholder text stating "this is a placeholder, currently under legal review." Final counsel-reviewed content must replace this before public launch (`tos_version_accepted` value will need to be bumped at that point).
...
- **Phase 55d:** Fix ORCID OAuth callback back-button trap — ✅ COMPLETE (added `exchangedRef` ref guard in `OrcidCallback.jsx` to prevent double code exchange when `refreshUser()` changes `user` effect dependency and triggers effect re-run; both `navigate` calls already had `replace: true`; no other callback-style components found in audit. Discovered during production smoke test on orcaconcepts.org after DNS cutover.)
